...
async function deserializeAccountInfo(
  ...
  const data = Buffer.from(
    await decompress(Buffer.from(accountInfo.data[0], "base64"))
  );
...
export async function deserializeAccountInfosMap(
  ...
  accountInfosMap.forEach(async (value, key) => {
    promises.push(
      deserializeAccountInfo(value).then(
        (accountInfo) => (
          deserializedAccountInfoMap.set(key, accountInfo), void 0
        )
      )
    );
  });
